...
Usage:
    ...
    python scripts/upload_inbox.py ~/papers/ http://gomer:8766

    # Upload without auto-ingest (trigger manually in Claude later):
    python scripts/upload_inbox.py ~/papers/ http://gomer:8766 --no-ingest

    # Upload PPTX slides:
    python scripts/upload_inbox.py ~/slides/ http://gomer:8766 --glob "*.pptx"

    # Upload a single file:
    python scripts/upload_inbox.py paper.pdf http://gomer:8766

    # Recurse into subdirectories:
    python scripts/upload_inbox.py ~/library/ http://gomer:8766 --recursive

    # Custom timeout (seconds per file, default 120):
    python scripts/upload_inbox.py ~/papers/ http://gomer:8766 --timeout 300

Prerequisites: pip install httpx  (already a transitive dep of lab-corpus-mcp)
"""
...
def _require_httpx():
    try:
        ...
        sys.exit(
            ...
            "Or from the lab-corpus-mcp dev env:  pip install -e '.[dev]'"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("server",
                   help="lab-corpus-mcp HTTP base URL, e.g. http://gomer:8766")
...
def main() -> None:
    ...
    with httpx.Client() as client:
        for i, filepath in enumerate(files, 1):
            ...
            # Trigger ingest automatically on the last file when --ingest is set.
            is_last = i == len(files)
            ...
            try:
                ...
                if result.get("saved"):
                    ...
                    print(f"OK{suffix}")
                    if job_id:
                        # Print once; repeated below in summary.
                        _job_id = job_id
                else:
                ...
                break  # no point continuing if the server is down
    ...
    if not args.ingest:
        print()
        print("Files are in the server inbox. To start MinerU parsing, call:")
        print("  ingest_inbox()  in Claude Desktop, or")
        print(f"  curl -X POST '{upload_url}?ingest=true' \\")
        print(f"       -F 'file=@/dev/null'  # triggers on non-empty inbox")
    else:
        # job_id was printed inline; remind the user how to check.
        print()
        print("MinerU ingest job submitted. Track progress with:")
        print("  job_status(<job_id>) via Claude, or job_list() to see recent jobs.")
